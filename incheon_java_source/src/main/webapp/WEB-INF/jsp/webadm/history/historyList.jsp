...
<%
 	...
 	String strLinkPage   = CommDef.ADM_PATH + "/history/historyList.do"; // request.getRequestURL().toString(); // 현재 페이지
	...
	<div id="contain">
		...
		<div id="content">
			...
			<div style="text-align:right;">
				<button type="button" class="__btn1 type3" onclick="javascript:fWrite();">연혁 등록</button>
			</div>
			<br/>
			<table class="__tbl-list __tbl-respond">
				...
				<thead>
					<tr>
						...
						<th scope="col">년도</th>
						<th scope="col">월,일 내용</th>
						<th class="subject" scope="col">내용</th>
						<th scope="col">순서</th>
...
%>     
					<tr>
						...
						<td><%=CommonUtil.nvl(rsMap.get("YEAR_TEXT")) %></td>
						<td><%=CommonUtil.nvl(rsMap.get("MONTH_TEXT")) %></td>
...
%>              
          <tr>
            <td colspan="7"><%=CommDef.Message.NO_DATA %> </td>
          </tr>       
<%  } %> 
...
<script>
   ...
   function fWrite()
   {
	  ...
	  vObj.seq.value = '';
	  vObj.action = "<%=CommDef.ADM_PATH %>/history/historyWrite.do";
   ...
   function fDelete(strSeq)
   {
	  ...
	  vObj.mode.value = "del";
	  vObj.seq.value = strSeq; 
	  vObj.action = "<%=CommDef.ADM_PATH %>/history/historyDelete.do";

...
<%
	...
	String[] chartColorArrayValue = {"#FF0F00" , "#FF6600", "#FF9E01", "#FCD202", "#F8FF01", "#B0DE09","#04D215", "#0D8ECF", "#0D52D1", "#2A0CD0", "#8A0CCF"};
	String[] chartColorArray = new String[listcount];
	
	for (int i = 0; i < listcount; i++) {
		if (i < 11) {
			chartColorArray[i] = chartColorArrayValue[i];
		} else {
			String chartNum = Integer.toString(i);			
			int len = chartNum.length();
			int chartNumSize = Integer.parseInt(chartNum.substring(len-1, len));
			chartColorArray[i] = chartColorArrayValue[chartNumSize];
		}
	}

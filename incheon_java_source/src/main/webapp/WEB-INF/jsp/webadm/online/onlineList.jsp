...
<%
 	...
 	String strLinkPage   = CommDef.ADM_PATH + "/online/onlineList.do"; // request.getRequestURL().toString(); // 현재 페이지
...
%>     
					<tr>
						...
						<td><%=CommonUtil.nvl(rsMap.get("NAME")) %></td>
						<td><%=CommonUtil.nvl(rsMap.get("TEL")) %></td>
						<td><%=CommonUtil.nvl(rsMap.get("EMAIL")) %></td>
						<td><%=CommonUtil.getDateFormat(rsMap.get("REG_DT"), "-") %></td>	
						<td><%=CommonUtil.getYNReplytext(rsMap.get("STATE").toString()) %></td>
						<td>
							<% if (!CommonUtil.nvl(rsMap.get("RE_REGDT")).equals("")) { %>
								<%=CommonUtil.getDateFormat(rsMap.get("RE_REGDT"), "-") %>
							<% } %>
						</td>	
						<td>
							<a href="javascript:fModify('<%=CommonUtil.nvl(rsMap.get("SEQ"))%>');" class="__btn1 type2">보기</a>
...
<script>
   ...
   function fModify(strSeq)
   {
	  ...
	  vObj.action = "<%=CommDef.ADM_PATH %>/online/onlineWrite.do";
   ...
   function fDelete(strSeq)
   {
	  ...
	  vObj.mode.value = "del";
	  vObj.seq.value = strSeq; 
	  vObj.action = "<%=CommDef.ADM_PATH %>/online/onlineDelete.do";

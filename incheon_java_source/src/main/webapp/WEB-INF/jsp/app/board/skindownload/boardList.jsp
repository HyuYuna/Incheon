		...
		<div id="sb-list">
			...
			%>
					<tr class="tr_notice">
						...
						<td class="down">
						<% if (!CommonUtil.nvl(rsNoticeMap.get("FILE_REALNM")).equals("")) { %>
							<% if (CommonUtil.getNullInt(brdMgrMap.get("READ_LEVEL_CD"), 0) > userLevel) { %>
								<a href="javascript:alert('다운로드 권한이 없습니다.');">
							<% } else { %>
								<a href="javascript:download(<%=CommonUtil.getNullTrans(rsNoticeMap.get("FILE_NO"))%>);">
							<% } %><img src="<%=CommDef.APP_CONTENTS %>/images/sb_ico_listdown.png" /></a>
						<% } %>
						</td>
					</tr>
			<%
			...
			%>
					<tr>
						...
						<td class="down">
						<% if (!CommonUtil.nvl(rsMap.get("FILE_REALNM")).equals("")) { %>
							<% if (CommonUtil.getNullInt(brdMgrMap.get("READ_LEVEL_CD"), 0) > userLevel) { %>
								<a href="javascript:alert('다운로드 권한이 없습니다.');">
							<% } else { %>
								<a href="javascript:download(<%=CommonUtil.getNullTrans(rsMap.get("FILE_NO"))%>);">
							<% } %><img src="<%=CommDef.APP_CONTENTS %>/images/sb_ico_listdown.png" /></a>
						<% } %>
						</td>
					</tr>

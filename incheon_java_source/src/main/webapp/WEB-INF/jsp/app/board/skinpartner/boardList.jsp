		...
		<ul id="sb-partner">
			...
			<li>
				...
				<%
						...
						if (!CommonUtil.nvl(rsMap.get("URL_TEXT")).equals("")) {
				%>
						<a href="<%=CommonUtil.nvl(rsMap.get("URL_TEXT")) %>" class="link" target="_blank"></a>
				<% } else { %>
				...
				<div class="tmb">
					...
					%>
						<img src="<%=CommonUtil.getThumnailSrc(rsMap.get("FILE_REALNM")) %>"/>
					<% } else { %>
						<img src="http://wimg.kr/<%=CommonUtil.nvl(brdMgrMap.get("THM_WIDTH_SIZE")) %>x<%=CommonUtil.nvl(brdMgrMap.get("THM_HEIGHT_SIZE")) %>&l=false" />
					<% } %>
				</div>
				<div class="infobox">
					<span class="sbj"><%=CommonUtil.getStrCut(CommonUtil.nvl(rsMap.get("TITLE")),150) %></span>
					<a href="<%=CommonUtil.nvl(rsMap.get("URL_TEXT")) %>" class="url" target="_blank"><%=CommonUtil.nvl(rsMap.get("URL_TEXT")) %></a>
				</div>
			</li>
			<%
					iSeqNo--;

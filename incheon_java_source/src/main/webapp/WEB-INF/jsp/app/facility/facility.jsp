			                ...
			                <form action="<%=strLinkPage %>" method="get">
			                    ...
			                    <fieldset>
			                        ...
			                        <select name="ca1" id="ca1">
			                        	<option value=''>시설 분류를 선택해주세요.</option>
			                        </select>

			                        <input type="text" name="shtext" id="shtext" class="inp" placeholder="시설명을 입력해주세요." />
			                        <div class="tac mt20">
			                            <button type="submit" class="sbm bbtn1" id="searchBtn">복지시설 검색</button>
			                        </div>
			                    </fieldset>
			                </form>
			            </dd>
...
var app = {
	...
	btnEvent : function() {
		...
		$('#searchBtn').on('click', function() {
			...
			if (ca1value == "") { //시설 분류 값이 없을 경우
				$('#menuno').val(<%=CommonUtil.nvl(reqMap.get("menuno"), "")%>);
			} else {  //시설 분류값이 있을 경우
				$('#menuno').val($('#ca1 option:selected').attr('data-menuno'));
			}
	...
	//시설 카테고리
	selectCategory : function(guvalue) {
	    ...
	    $.ajax({
	        ...
	        success : function(data) {
	        	...
	        	optionvalue += "<option value=''>시설 분류를 선택해주세요.</option>";

	        	if (data.length > 0) {
					for (var i = 0; i < data.length; i++) {
						optionvalue += "<option value='" + data[i]['CA1'] + "' data-menuno='"  + data[i]['MENUNO'] + "'>" + data[i]['CA1_NM']  + "</option>";
					}
	        	}

	        	$('#ca1').append(optionvalue);
